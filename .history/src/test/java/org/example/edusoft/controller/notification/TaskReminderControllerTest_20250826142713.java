...
public class TaskReminderControllerTest {
    ...
    **目标:** 验证当任务不存在时，服务层抛出异常，控制器能正确处理。
    ...
    ## getUserTaskReminders(), getUncompletedTaskReminders(), getCompletedTaskReminders() 测试

    ### 正常情况测试 (Positive Tests)
    **目标:** 验证根据用户ID成功获取不同状态的任务列表。
    ```java
    @Test
    ...
    ### 异常情况测试 (Negative Test)
    **目标:** 验证当用户ID无效或服务层出现问题时，控制器能正确处理。
    ```java
    @Test
    ...
    ## markTaskAsCompleted() & markTaskAsUncompleted() 测试
