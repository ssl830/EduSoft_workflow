            ...
            <table class="records-table">
              ...
              <tbody>
                <tr v-for="record in getCourseExercises(course.id)" :key="record.id">
                  ...
                  <td>{{ record.score !== undefined ? record.score : '-' }}</td>                  <td class="actions-cell actions-cell-center">                    <button 
...
const fetchCoursesList = async () => {
  try {
    ...
    records.value.forEach(record => {
      console.log('处理记录:', record.title, '课程ID:', record.courseId);
      if (record.courseId) {
        // 添加到对应课程的练习列表
        ...
        if (!coursesWithExercises[record.courseId]) {
          const matchedCourse = courses.find(c => c.id === record.courseId);
          console.log('查找课程ID', record.courseId, '是否找到:', matchedCourse);
          if (matchedCourse) {
    ...
    // 转换为数组
    coursesList.value = Object.values(coursesWithExercises);
...
const exportCourseRecords = async (courseId: number) => {
  try {
    ...
    console.log(`成功导出课程学习记录: ${courseName}_学习记录_${timestamp}.xlsx`);
...
const exportCoursePracticeRecords = async (courseId: number) => {
  try {
    ...
    const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
    const courseName = coursesList.value.find(c => c.id === courseId)?.name || '课程';
    handleFileDownload(response.data, `${courseName}_练习记录_${timestamp}.xlsx`);
    
    console.log(`成功导出课程练习记录: ${courseName}_练习记录_${timestamp}.xlsx`);
...
const viewSubmissionReport = async (recordIdOrSubmissionId: string | undefined, record?: any) => {
  ...
  try {
    ...
    // 先尝试使用submission ID获取报告
    let response;
    try {
      response = await StudyRecordsApi.getSubmissionReport(submissionId);
    } catch (submissionError: any) {
      console.log('使用submission ID失败，尝试使用练习ID:', submissionError);
      
      // 如果submission ID失败，且我们有练习记录，尝试使用练习详情API
      if (record && record.id) {
        response = await StudyRecordsApi.getStudentExerciseRecordDetail(record.id);
      } else {
        throw submissionError;
      }
    }    console.log('练习提交报告API响应:', response);
    console.log('响应的完整结构:', JSON.stringify(response.data, null, 2));
    
    if (response.data && response.data.code === 200) {
      // 修复数据访问路径 - 检查是否有嵌套的data字段
      let reportData;
      if (response.data.data !== undefined) {
        reportData = response.data.data;
      } else {
        // 如果没有嵌套的data字段，直接使用response.data
        reportData = response.data;
      }
      
      console.log('API返回的原始数据:', reportData);
      console.log('数据类型:', typeof reportData);
      console.log('数据键名:', Object.keys(reportData || {}));
      
      // 检查数据是否存在 - 更严格的检查
      if (!reportData || (typeof reportData === 'object' && Object.keys(reportData).length === 0)) {
        console.error('后端返回空数据，可能的原因:');
        console.error('1. submissionId 在数据库中不存在');
        console.error('2. 当前学生没有权限访问该记录');
        console.error('3. 后端查询逻辑有问题');
        throw new Error('API返回的数据为空，请检查提交ID是否正确');
      }
      ...
      // 立即验证数据结构
      console.log('设置后立即检查 - 数据存在:', !!submissionReportModal.value.data);
      console.log('设置后立即检查 - 提交信息存在:', !!submissionReportModal.value.data?.submissionInfo);      // 强制更新响应式数据
      submissionReportModal.value.data = { ...reportData };
      console.log('成功设置练习提交报告数据:', submissionReportModal.value.data);
      
      // 立即验证数据结构
      console.log('设置后立即检查 - 数据存在:', !!submissionReportModal.value.data);
      console.log('设置后立即检查 - 提交信息存在:', !!submissionReportModal.value.data?.submissionInfo);
      
      // 验证数据结构
      if (reportData.submissionInfo) {
        console.log('提交信息:', reportData.submissionInfo);
      }
      if (reportData.questions) {
        console.log('题目数量:', reportData.questions.length);
      }
      if (reportData.scoreDistribution) {
        console.log('分数分布:', reportData.scoreDistribution);
      }
    } else {
      console.error('API调用失败，响应代码:', response.data?.code);
...
const downloadReport = async (recordIdOrSubmissionId: string) => {
  try {
    ...
    if (!submissionId) {
      console.warn('未找到提交记录ID，将使用练习ID作为替代。记录/练习ID:', recordIdOrSubmissionId);
      submissionId = recordIdOrSubmissionId;
    }
    
    console.log('准备下载报告，使用ID:', submissionId);
    ...
    // 传递响应对象以便从响应头获取文件名
    handleFileDownload(response.data, filename, response);
...
const fetchRecords = async () => {
  try {
    ...
    try {
      ...
      if (practiceResponse.data && practiceResponse.data.code === 200 && Array.isArray(practiceResponse.data.data)) {
        practiceRecords = practiceResponse.data.data;
        console.log('成功解析练习记录:', practiceRecords.length, '条');
          practiceRecords.forEach((record: any) => {
          // 确保submission_id总是有值，首选record.id
          const submissionId = record.id?.toString() || 
                             record.practiceId?.toString() || 
                             record.submissionId?.toString() || 
                             `practice_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
          ...
          allRecords.push({
            ...
            submission_id: submissionId, // 使用我们确保有值的submissionId
            student_name: record.className || record.studentName || '',
          });
        });
      } else {
        console.warn('练习记录API响应格式不正确:', practiceResponse.data);
      }
